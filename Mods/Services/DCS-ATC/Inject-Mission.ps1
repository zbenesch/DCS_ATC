...
# ── 1. Merge airfields.lua + ATC_Script.lua ──────────────────────────────────
$airfieldsContent = Get-Content $airfieldsPath -Raw -Encoding UTF8
$atcScriptContent = Get-Content $atcScriptPath  -Raw -Encoding UTF8

$merged = @"
-- Auto-generated by Inject-Mission.ps1 -- DO NOT EDIT INSIDE .MIZ
-- airfields.lua merged here so runway data is always available:
$airfieldsContent
...
$mergedBytes = [System.Text.Encoding]::UTF8.GetBytes($merged)

Write-Host "Opening: $MizPath" -ForegroundColor Cyan

$zip = [System.IO.Compression.ZipFile]::Open($MizPath, 'Update')
try {
    # Inject merged script as l10n/DEFAULT/ATC_Script.lua
    $scriptEntry = "l10n/DEFAULT/ATC_Script.lua"
    $existing = $zip.GetEntry($scriptEntry)
    if ($existing) { $existing.Delete() }

    $entry = $zip.CreateEntry($scriptEntry, [System.IO.Compression.CompressionLevel]::Optimal)
    $stream = $entry.Open()
    try { $stream.Write($mergedBytes, 0, $mergedBytes.Length) }
    finally { $stream.Dispose() }
    Write-Host "Injected merged ATC_Script.lua (airfields + script, $($mergedBytes.Length) bytes)." -ForegroundColor Green
